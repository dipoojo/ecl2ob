...
class LSTM_unroll_dynamic_seq(nn.Module):
    def __init__(self, num_classes, input_size, hidden_size, num_layers, 
                 seq_length, w, l_1, l_2, l_3):
        
        super(LSTM_unroll_dynamic_seq, self).__init__()
    ...
    def forward(self, x, p_1, mode="train", calib = True):
        ...
        action_00 = torch.zeros(x.size(0), 5, x.size(2), device = x.device)
        action_0 = torch.zeros(x.size(0), 1, x.size(2), device = x.device)
        xtilde = torch.zeros(x.size(0), 1, x.size(2), device = x.device) #for calculating rho
        action_sequence = []
        #input_sequence = []
        ...
        if (mode not in ["train", "val"]):
            raise NotImplementedError
        # changed seq_length to self.seq_length for now
        if (self.seq_length != self.seq_length and mode == "train"):
            print("The length of sequence is changed, be careful!!!!")
            assert 0
        #print(x[:,:,0].unsqueeze(1).shape)   
        for i in range(self.seq_length):
            
            x_i = x[:,i:i+5,:] #.unsqueeze(1)
            #print(i)
            #print(x_i.shape)
            #print(type(x_i))
            #print(f'action shape is {action_00.shape}')
            input_i = torch.cat([x_i, action_00], 2)
            ula, (h_out, c_out) = self.lstm(input_i, (h_0, c_0))
            #input_sequence = torch.cat(input_sequence, x_i)
            #input_array = np.array(input_sequence)
            #input_array = input_array.reshape(-1,1)
            #print(f' ula shape is {ula.shape}')
            ula = ula[:,-1,:].view(-1, self.hidden_size)
            out = self.fc(ula).unsqueeze(1)
            #print(f'{out.shape} outshape')
            #p_2 = np.expand_dims(p_1, axis=1)
            #p_2 = p_1.unsqueeze(1)
            ...
            if i < 1:
                # The initial action directly follow the demand
                action_0 = x_i[:,-1,:].unsqueeze(1)
                xtilde = x_i[:,-1,:].unsqueeze(1).detach()
                #print(f'action o less than 1 {action_0.shape}')
                #print('action0')
                #print(action_0.shape)
            else:
                action_prev = action_sequence[-1]
                if calib:
                    input_array = x[:,i+1:5+i+1,:].detach()
                    #print('inputarray')
                    #print( i)
                    #print(f'input array shape is {input_array.shape}')
                    xtilde = torch.cat([xtilde,out.detach()],1)
                    ...
                    #print(f'xtilde is {xtilde}')
                    #print(f'action_sequence is {action_sequence}')
                    optimal_action, optimal_cost = calculate_offline_optimal_dynamic(x[:,5:5+i,:], x[:,4,:], switch_weight=10)
                    #print(f'optimal_action.shape is {optimal_action.shape}')
                    #print(f'input array is {input_array}')
                    #print(f'optimal action is {optimal_action}')
                    #print(f'optimal_cost is {optimal_cost}')
                    ...
                    #print(f'self.l_2 is {self.l_2}')
                #if calib:
                    action_0 = (1 + self.w*self.l_2)*x[:,4+i,:].unsqueeze(1) + self.w*self.l_1*action_prev + self.w*self.l_3*out
                    action_0 /= 1 + self.w*(self.l_1 + self.l_2 + self.l_3)
                    #print(f'for iter {i} action_o is {action_0.shape}')
                else:
                    action_0 = out
                    #print(f'action_o is {action_0[1:5,0]}')
            #print(f'after action {action_0.shape}')
            action_sequence.append(action_0)
            action_00 = action_00[:,1:,:]
            action_00 = torch.cat((action_00, action_0),1)
            #print("")
            #print("")
