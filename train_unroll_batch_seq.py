...
from utils.model import LSTM_unroll, LSTM_unroll_dynamic, LSTM_unroll_dynamic_seq
...
if __name__ == "__main__":
    ...
    for input_dict in [ecl20]: # [pureml, robd, mlrobd, ecl20, ecl20b]:    
        ...
        train_seq = sliding_windows(train_raw, window_size)
        val_seq = sliding_windows(val_raw, window_size)
        ...
        for i in range(1,5):
            temp_seq[i] = sliding_windows(test_raw[i], window_size)
            op_temp_cost_array[i] = optimal_cost_calc(test_raw[i], window_size)
        #temp_seq = sliding_windows(temp_raw, seq_length)
            bsize[i] = temp_seq[i].shape[0]
        #print(f'bsize is {bsize}')

        op_val_cost_array = optimal_cost_calc(val_raw, window_size)
        ...
        lstm_dynamic = LSTM_unroll_dynamic_seq(num_classes, input_size, hidden_size, num_layers, 
                        seq_length, w, l_1, l_2, l_3)
